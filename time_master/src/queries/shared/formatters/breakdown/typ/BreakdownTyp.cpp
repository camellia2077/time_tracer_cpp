...
std::string BreakdownTyp::format(const ProjectTree& tree, long long total_duration, int avg_days) const {
    std::stringstream ss;
...
void BreakdownTyp::generate_sorted_output(std::stringstream& ss, const ProjectNode& node, int indent, int avg_days) const {
    // 对子节点按时间排序
