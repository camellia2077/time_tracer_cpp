// generator/components/day_generator.cpp
 ...
 */
void DayGenerator::GenerateForDay(std::string& log_content, int month, int day, bool is_nosleep_day) {
    // 1. 生成日期标题
    std::format_to(std::back_inserter(log_content), "{:02}{:02}\n", month, day);

    // 2. 委托给 RemarkGenerator，尝试生成备注
    if (auto remark = remark_generator_->TryGenerate()) {
        std::format_to(std::back_inserter(log_content), "{}\n", *remark);
    }

    // 3. 委托给 EventGenerator，生成当天的所有活动
    event_generator_->GenerateEventsForDay(log_content, is_nosleep_day);
}
