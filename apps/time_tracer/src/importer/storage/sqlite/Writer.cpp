...
long long Writer::get_or_create_project_id(const std::string& project_path) {
    ...
    for (const auto& part : parts) {
        ...
        sqlite3_reset(stmt_select_project_id_); 
        sqlite3_bind_text(stmt_select_project_id_, 1, part.c_str(), -1, SQLITE_TRANSIENT); 
        if (current_parent_id == 0) {
            sqlite3_bind_null(stmt_select_project_id_, 2); 
        } else {
            sqlite3_bind_int64(stmt_select_project_id_, 2, current_parent_id); 
        }

        long long found_id = 0;
        if (sqlite3_step(stmt_select_project_id_) == SQLITE_ROW) { 
            found_id = sqlite3_column_int64(stmt_select_project_id_, 0); 
        }
        ...
        if (found_id > 0) {
            ...
            sqlite3_bind_text(stmt_insert_project_, 1, part.c_str(), -1, SQLITE_TRANSIENT); 
            if (current_parent_id == 0) {
            ...
            if (sqlite3_step(stmt_insert_project_) != SQLITE_DONE) { 
                std::cerr << "Error inserting project: " << sqlite3_errmsg(db_) << std::endl; 
                return 0; 
            }
            current_parent_id = sqlite3_last_insert_rowid(db_); 
        }
