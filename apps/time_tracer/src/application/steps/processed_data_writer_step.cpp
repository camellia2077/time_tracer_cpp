// application/steps/processed_data_writer_step.cpp
#include "application/steps/processed_data_writer_step.hpp"
#include "application/utils/processed_data_writer.hpp"

namespace core::pipeline {

ProcessedDataWriterStep::ProcessedDataWriterStep(
    std::shared_ptr<core::interfaces::ILogSerializer> serializer)
    : serializer_(std::move(serializer)) {}

bool ProcessedDataWriterStep::Execute(PipelineContext &context) {
  context.notifier->NotifyInfo("Step: Processed Data Writer");

  auto new_files = ProcessedDataWriter::Write(
      context.result.processed_data, context.config.output_root_,
      *context.file_system, *context.notifier, *serializer_);

  context.state.generated_files.insert(context.state.generated_files.end(),
                                       new_files.begin(), new_files.end());

  if (new_files.empty() && !context.result.processed_data.empty()) {
    context.notifier->NotifyWarning(
        "[Pipeline] 警告: 数据存在但未生成文件 (可能写入失败)");
  } else {
    context.notifier->NotifySuccess("JSON 数据已安全落盘");
  }

  return true;
}

} // namespace core::pipeline
