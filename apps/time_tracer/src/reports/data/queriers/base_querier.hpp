...
template <typename ReportDataType, typename QueryParamType> class BaseQuerier {
public:
  // [修改] 注入 Repository 接口
  ...
  virtual ReportDataType fetch_data() {
    ...
    data.project_stats_ = repo_.get_aggregated_project_stats(start, end);

    // 计算总时长
    long long total = 0;
    for (auto &p : data.project_stats_)
      total += p.second;
    data.total_duration_ = total;
