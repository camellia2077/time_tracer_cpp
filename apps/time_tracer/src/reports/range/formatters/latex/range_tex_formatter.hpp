...
public:
    ...
    // 满足基类模板接口要求
    std::map<std::string, std::string> get_keyword_colors() const { return {}; }
 ...
 */
class RangeTexFormatter : public BaseTexFormatter<RangeReportData, RangeTexFormatterConfig> {
public:
...
private:
    ...
    void display_summary(std::stringstream& ss, const RangeReportData& data) const;
};
