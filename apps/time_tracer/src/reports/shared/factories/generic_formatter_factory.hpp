...
template <typename ReportDataType> class GenericFormatterFactory {
public:
  using ConfigType = typename ReportTraits<ReportDataType>::ConfigType;
  using FormatterType = IReportFormatter<ReportDataType>;
  using Creator =
      std::function<std::unique_ptr<FormatterType>(const ConfigType &)>;
...
public:
  ...
  static void register_dll_formatter(ReportFormat format,
                                     const std::string &dll_base_name) {

    auto creator_lambda =
        [dll_base_name](
            const ConfigType &config) -> std::unique_ptr<FormatterType> {
      // 1. 自动补全 DLL 后缀
      std::string dll_path = dll_base_name;
#ifdef _WIN32
      if (dll_path.find(".dll") == std::string::npos)
        dll_path += ".dll";
#else
      if (dll_path.find("lib") != 0 && dll_path.find("/") == std::string::npos)
        dll_path = "lib" + dll_path;
      if (dll_path.find(".so") == std::string::npos)
        dll_path += ".so";
#endif

      // 2. [核心重构] 使用 ConfigTomlSerializer 生成 TOML 字符串
      // 变量名改为 serialized_toml_config 以消除歧义
      std::string serialized_toml_config =
          ConfigTomlSerializer::to_toml_string(config);

      // 3. 创建 Wrapper
      try {
        auto wrapper = std::make_unique<DllFormatterWrapper<ReportDataType>>(
            dll_path, serialized_toml_config);
        if (!wrapper) {
          throw std::runtime_error("Unknown error creating DLL wrapper");
        }
        return wrapper;
      } catch (const std::exception &e) {
        std::cerr << "[Factory] Failed to load DLL formatter '" << dll_path
                  << "': " << e.what() << std::endl;
        throw;
      }
    };
  ...
  static std::unique_ptr<FormatterType> create(ReportFormat format,
                                               const ConfigType &config) {
    auto &registry = get_registry();
    if (registry.find(format) == registry.end()) {
      throw std::runtime_error("No formatter registered for format: " +
