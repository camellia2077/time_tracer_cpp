// reports/shared/formatters/templates/base_tex_formatter.hpp
...
protected:
  ...
  virtual std::string generate_preamble() const {
    return TexUtils::get_tex_preamble(
        ...
        config_->GetBaseFontSize(), config_->GetMarginIn(),
        get_keyword_colors());
  }
  ...
  virtual void format_project_tree_section(std::stringstream &ss,
    ...
    ss << "\\fontsize{" << title_size << "}{" << title_size * 1.2
       << "}\\selectfont";
    ss << "\\section*{"
       << TexUtils::escape_latex(config_->GetProjectBreakdownLabel()) << "}";
    ss << "}\n\n";
