// reports/period/formatters/markdown/period_md_formatter.cpp
#include "reports/period/formatters/markdown/period_md_formatter.hpp"
#include <format>
...
extern "C" {
    __declspec(dllexport) FormatterHandle create_formatter(const char* config_toml) {
        try {
            ...
            auto md_config = std::make_shared<reporting::PeriodMdConfig>(config_tbl);
            auto formatter = new reporting::PeriodMdFormatter(md_config);
    ...
    __declspec(dllexport) void destroy_formatter(FormatterHandle handle) {
        if (handle) {
            delete static_cast<reporting::PeriodMdFormatter*>(handle);
        }
    }
    ...
    __declspec(dllexport) const char* format_report(FormatterHandle handle, const PeriodReportData& data) {
        if (handle) {
            auto* formatter = static_cast<reporting::PeriodMdFormatter*>(handle);
