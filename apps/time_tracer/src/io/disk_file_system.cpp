...
std::string DiskFileSystem::ReadContent(const fs::path &path) {
  if (!fs::exists(path)) {
...
std::vector<fs::path> DiskFileSystem::FindFiles(const fs::path &root,
  ...
  try {
    ...
    // 这里选择打印错误，或者也可以选择抛出
    std::cerr << kRedColor << "Filesystem error accessing directory " << root
              << ": " << e.what() << kResetColor << std::endl;
  }
...
DiskFileSystem::ResolveFilePaths(const std::vector<std::string> &inputs,
                                 const std::string &ext) {
  ...
  for (const std::string &path_str : inputs) {
    fs::path p(path_str);
    if (!fs::exists(p)) {
      std::cerr << kYellowColor << "Warning: Path does not exist: " << path_str
    ...
    if (fs::is_regular_file(p)) {
      ...
      // 复用自身的 find_files 方法
      auto found_paths = FindFiles(p, ext);
...
void DiskFileSystem::WriteContent(const fs::path &path,
...
void DiskFileSystem::CreateDirectories(const fs::path &path) {
  try {
...
bool DiskFileSystem::IsDirectory(const fs::path &path) {
  std::error_code ec;
  return fs::is_directory(path, ec);
}

bool DiskFileSystem::IsRegularFile(const fs::path &path) {
  std::error_code ec;
