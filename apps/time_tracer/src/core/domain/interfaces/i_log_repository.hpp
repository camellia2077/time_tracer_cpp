...
public:
  ...
  // 保存日志到持久化存储
  virtual void
  Save(const std::map<std::string, std::vector<DailyLog>> &logs) = 0;

  // 按日期范围查询
  virtual std::vector<DailyLog> FindByDate(const std::string &date) = 0;

  // 按日期范围批量查询
  virtual std::map<std::string, std::vector<DailyLog>>
  FindByDateRange(const std::string &start_date,
