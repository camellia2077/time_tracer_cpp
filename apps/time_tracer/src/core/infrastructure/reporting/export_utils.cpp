...
std::optional<ReportFormatDetails> get_report_format_details(ReportFormat format, core::interfaces::IUserNotifier* notifier) {
    switch (format) {
        ...
        default:
            if (notifier) notifier->notify_error("错误: 不支持的导出格式。");
            return std::nullopt;
    }
}
...
void execute_export_task(
    ...
    try {
        ...
        if (files_created > 0) {
            ...
            notifier.notify_success("成功: 共创建 " + std::to_string(files_created) + " 个" + report_type_name_singular 
                      + "文件，已保存至: " + final_path.string());
        } else {
             notifier.notify_warning("信息: 没有可导出的" + report_type_name_singular + "内容。");
        }

    } catch (const std::exception& e) {
        notifier.notify_error("导出过程中发生错误: " + std::string(e.what()));
    }
}
