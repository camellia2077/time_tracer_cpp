...
LineRules::LineRules(const ConverterConfig &config) : config_(config) {
  ...
  for (const auto &pair : config.mapper_config_.text_mapping_)
    valid_event_keywords_.insert(pair.first);
  for (const auto &pair : config.mapper_config_.text_duration_mapping_)
    valid_event_keywords_.insert(pair.first);

  const auto &wake_vec = config.parser_config_.wake_keywords_;
  wake_keywords_.insert(wake_vec.begin(), wake_vec.end());

  for (const auto &pair : config.mapper_config_.top_parent_mapping_)
    valid_event_keywords_.insert(pair.first);
  for (const auto &pair : config.mapper_config_.initial_top_parents_)
    valid_event_keywords_.insert(pair.first);
}
...
bool LineRules::is_remark(const std::string &line) const {
  const std::string &prefix = config_.parser_config_.remark_prefix_;
  if (prefix.empty() || line.rfind(prefix, 0) != 0)
    return false;
  return !Trim(line.substr(prefix.length())).empty();
}

bool LineRules::is_valid_event_line(const std::string &line, int line_number,
  ...
  try {
    ...
    if (wake_keywords_.count(description) == 0 &&
        valid_event_keywords_.count(description) == 0) {
