...
void validateDateContinuity(const std::vector<DailyLog>& days, std::set<Error>& errors, DateCheckMode mode) {
    if (mode == DateCheckMode::None || days.empty()) return;

    // 获取基准年月 (从第一条数据)
    const DailyLog& first_day = days.front();
    std::string first_date = first_day.date; 
    ...
    for (const auto& day : days) {
        if (day.date.rfind(yyyy_mm, 0) == 0 && day.date.length() == 10) {
            days_found.insert(std::stoi(day.date.substr(8, 2)));
        }
    }
    ...
    for (int d = 1; d <= check_until; ++d) {
        if (days_found.find(d) == days_found.end()) {
            std::string date_str = yyyy_mm + "-" + (d < 10 ? "0" : "") + std::to_string(d);
            errors.insert({0, "Missing date: " + date_str, ErrorType::DateContinuity});
        }
    }
}
