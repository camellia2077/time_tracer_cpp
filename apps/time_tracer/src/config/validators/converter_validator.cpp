...
bool MainRule::validate(const toml::table &main_tbl,
  ...
  for (const auto &key : required_keys) {
    if (!main_tbl.contains(key)) {
      std::cerr << "[Validator] Error: Main config is missing required key: '"
                << key << "'" << std::endl;
      return false;
    }
  }
  ...
  if (!main_tbl["mappings_config_path"].is_string() ||
    ...
    std::cerr << "[Validator] Error: 'mappings_config_path' and "
                 "'duration_rules_config_path' must be strings."
              << std::endl;
    return false;
  }
  if (!main_tbl["top_parent_mapping"].is_table()) {
    std::cerr << "[Validator] Error: 'top_parent_mapping' must be a Table."
              << std::endl;
    return false;
  }
  if (!main_tbl["header_order"].is_array() ||
      !main_tbl["wake_keywords"].is_array()) {
    std::cerr << "[Validator] Error: 'header_order' and 'wake_keywords' must "
                 "be arrays."
              << std::endl;
    return false;
  }
  if (!main_tbl["remark_prefix"].is_string()) {
    std::cerr << "[Validator] Error: 'remark_prefix' must be a string."
              << std::endl;
    return false;
  }
...
bool DurationRule::validate(const toml::table &duration_tbl) const {
  ...
  for (const auto &[key, node] : *mappings) {
    if (!node.is_array()) {
      std::cerr << "[Validator] Error: In duration rules, the value for key '"
                << key.str() << "' must be an array." << std::endl;
      return false;
    }
    ...
    for (const auto &rule_node : rules_array) {
      if (!rule_node.is_table()) {
        std::cerr << "[Validator] Error: In duration rules for '" << key.str()
                  << "', each rule must be an inline table." << std::endl;
        return false;
      }
      const auto &rule = *rule_node.as_table();

      if (!rule.contains("less_than_minutes") || !rule.contains("value")) {
        std::cerr << "[Validator] Error: Rules must have 'less_than_minutes' "
                     "and 'value'."
                  << std::endl;
        return false;
      }
      ...
      if (!rule["value"].is_string()) {
        std::cerr << "[Validator] Error: 'value' must be a string."
                  << std::endl;
        return false;
      }
    }
