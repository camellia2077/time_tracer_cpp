...
// ============================================================================
static bool validate_stats_recursive(const toml::array &items_array,
  ...
  for (size_t i = 0; i < items_array.size(); ++i) {
    ...
    if (item.contains("sub_items")) {
      if (const toml::array *sub_arr = item["sub_items"].as_array()) {
        if (!validate_stats_recursive(*sub_arr, file_name,
...
// BaseStrategy 实现
// ============================================================================
bool BaseStrategy::validate(const toml::table &query_config,
...
bool BaseStrategy::validate_common_rules(const toml::table &query_config,
                                         const std::string &file_name) const {
    ...
    if (key_str == "keyword_colors") {
      if (!node.is_table()) {
        std::cerr << "[Validator] Error in " << file_name
                  << ": 'keyword_colors' must be a table." << std::endl;
        return false;
      }
...
bool BaseStrategy::validate_statistics_items(
  ...
  if (!items_node.is_array()) {
    std::cerr << "[Validator] Error in " << file_name
              << ": 'statistics_items' must be an array." << std::endl;
    return false;
  }
...
// DailyTexStrategy 实现
// ============================================================================
bool DailyTexStrategy::validate_specific_keys(
    const toml::table &query_config, const std::string &file_name) const {
...
// DailyTypStrategy 实现
// ============================================================================
bool DailyTypStrategy::validate_specific_keys(
    const toml::table &query_config, const std::string &file_name) const {
...
// DailyMdStrategy 实现
// ============================================================================
bool DailyMdStrategy::validate_specific_keys(
    const toml::table &query_config, const std::string &file_name) const {
...
// RangeStrategy 实现
// ============================================================================
bool RangeStrategy::validate_specific_keys(const toml::table &query_config,
                                           const std::string &file_name) const {
  bool is_valid = true;

  if (!query_config.contains("project_breakdown_label")) {
    std::cerr << "Error in " << file_name
              << ": Missing 'project_breakdown_label'" << std::endl;
    is_valid = false;
  }

  if (!query_config.contains("total_time_label")) {
    std::cerr << "Error in " << file_name << ": Missing 'total_time_label'"
              << std::endl;
    is_valid = false;
  }

  if (!query_config.contains("actual_days_label")) {
    std::cerr << "Error in " << file_name << ": Missing 'actual_days_label'"
              << std::endl;
    is_valid = false;
  }
...
// StrategyFactory 实现
// ============================================================================
std::unique_ptr<IQueryStrategy> StrategyFactory::create(ReportType type,
                                                        ReportFormat format) {
  switch (type) {
  case ReportType::Daily:
    switch (format) {
    case ReportFormat::Markdown:
      return std::make_unique<DailyMdStrategy>();
    case ReportFormat::LaTeX:
      return std::make_unique<DailyTexStrategy>();
    case ReportFormat::Typ:
      return std::make_unique<DailyTypStrategy>();
    default:
      throw std::invalid_argument("Unsupported format for Daily report");
    }
...
StrategyFactory::create_from_filename(const std::string &file_name) {
  ...
  else if (file_name.find("Typ") != std::string::npos)
    format = ReportFormat::Typ;
