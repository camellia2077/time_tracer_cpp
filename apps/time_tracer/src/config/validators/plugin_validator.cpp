// config/validators/plugin_validator.cpp
// 移动自: validator/plugins/facade/plugin_validator.cpp
...
bool PluginValidator::validate(
  ...
  for (const auto &plugin_name : expected_plugins) {
    ...
    if (!fs.Exists(dll_path_with_prefix) &&
        !fs.Exists(dll_path_without_prefix)) {
      std::cerr << "[Validator] Error: Required plugin '" << plugin_name
                << ".dll' not found in the plugins directory." << std::endl;
      all_found = false;
    } else {
      std::cout << "[Validator] -- Found required plugin: " << plugin_name
                << std::endl;
    }
  }
