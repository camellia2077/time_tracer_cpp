...
* **验证层重构 (Validator Overhaul)**:
* **基于结构体的逻辑验证 (Struct-based Validation)**: 废弃了基于 JSON 字符串的 `JsonValidator`，引入了直接操作内存领域对象 (`DailyLog`) 的 `LogicValidator`。
* **性能优化 (Performance)**: 移除了逻辑验证步骤中多余的“序列化 -> 反序列化”过程，避免了 CPU 资源的浪费，让验证逻辑回归领域模型。
* **规则解耦 (Rule Decoupling)**: 将日期连续性 (`DateRules`) 和活动数量检查 (`ActivityRules`) 迁移至 `validator/logic` 模块，实现了验证逻辑与数据交换格式（JSON）的彻底解耦。


* **流水线优化 (Pipeline Optimization)**:
* **延迟序列化 (Lazy Serialization)**: 修改了 `ProcessedDataWriter` 和流水线上下文 (`PipelineContext`)，移除了 `cached_json_outputs` 中间缓存。JSON 序列化现在仅在最终写入磁盘 (`ProcessedDataWriterStep`) 时即时发生，不再作为验证的前置条件。
* **数据流净化**: 确立了 `Txt -> Struct -> Validator -> (Optional JSON) -> DB` 的高效数据流，消除了 JSON 作为必经中间态的架构限制。


* **配置对象解耦 (Configuration Decoupling)**:
* 将臃肿的 `ConverterConfig` 拆分为细粒度的 `LogParserConfig`、`LogMapperConfig` 和 `LogLinkerConfig`，遵循接口隔离原则 (ISP)，确保组件只依赖其所需的配置。
* 重命名上述配置结构体（添加 `Log` 前缀）以解决与 CLI 层 `ParserConfig` 的命名冲突。


* **领域模型迁移 (Domain Model Migration)**:
* 将核心业务实体（`DailyLog`, `ActivityStats`, `BaseActivityRecord`）从 `common/model` 移动至 `core/domain/model`，明确了领域层边界，符合整洁架构设计。


* **逻辑提取与去重 (DRY Principle)**:
* 提取分散在各处的事件计算逻辑，封装为 `TimeUtils` 通用工具类，统一了时间格式化和时长计算标准。


* **依赖倒置实现 (DIP)**:
* 引入 `IParser` 抽象接口，解耦了 `ConverterService` 与具体解析器 (`TextParser`) 的强绑定关系。



### 修复与适配 (Fixes & Adaptations)

* **组件适配**: 更新 `ActivityMapper`、`DayProcessor`、`LogLinker` 和 `TextParser` 以适配新的嵌套配置结构和命名空间。
* **CLI 编译修复**: 修正了 `ExportCommand` 和 `QueryCommand` 中对时间工具函数的调用，显式添加了 `TimeUtils::` 命名空间限定符。
* **工厂类修复**: 修复了 `ConverterConfigFactory` 头文件自我包含导致的循环依赖问题，并正确分离了声明与实现。
* **导入器路径修正**: 更新了 `Importer` 层中 `time_sheet_data.hpp` 的头文件引用路径，以匹配新的领域模型位置。
...
* **依赖倒置 (Dependency Inversion)**: 全面解耦了 `Core` 层与 `Serializer`、`Converter` 具体实现层的依赖关系。Core 层现在仅依赖抽象接口，不再直接依赖具体类。
* **依赖注入 (Dependency Injection)**:
* 实现了从 `CliApplication` (Composition Root) 到 `Core` 服务的完整依赖注入链路。
* **[新增]** 完成了 `Importer` 子系统内部的组件依赖解耦，实现了细粒度的依赖注入。



### 核心组件更新 (Component Updates)

#### 1. Serializer Layer (序列化层)

* **接口实现**: `JsonSerializer` 类现在实现了 `core::interfaces::ILogSerializer` 接口。
* **实例方法化**: 移除了原有的静态方法 (`static`) 设计，改为实例方法，以支持多态调用和接口继承。
* **功能对齐**: 提供了标准的 `serialize` 和 `deserialize` 接口实现，底层继续复用 `yyjson` 高性能解析逻辑。

#### 2. Converter Layer (转换层)

* **接口实现**: `LogProcessor` 类现在实现了 `core::interfaces::ILogConverter` 接口。
* **无状态设计 (Stateless Design)**: 重构了 `LogProcessor`，使其不再持有 `ConverterConfig` 状态。配置对象现在作为 `convert` 方法的参数在运行时动态传入，提高了组件的复用性和测试性。
* **错误处理**: 标准化了 `LogProcessingResult` 返回结构，统一了转换结果与状态的传递。

#### 3. Validator Layer (验证层)

* **领域模型验证 (Struct-based Validation)**: 彻底移除了基于 JSON 字符串的验证逻辑。`LogicValidator` 现在直接对内存中的 `DailyLog` 领域对象进行检查，消除了验证阶段对序列化器的隐式依赖。
* **组件拆分**:
* `TextValidator`: 专注于源文件 (`.txt`) 的格式与结构检查，支持通过 `ConverterConfig` 注入规则。
* `LogicValidator`: 专注于业务逻辑（如日期连续性、活动完整性）检查，支持 `DateCheckMode` 配置。


* **统一错误报告**: 引入 `validator_utils`，统一了所有验证器的错误定义 (`Error` 结构体) 和报告格式化逻辑。

#### 4. Core Layer (核心层)

* **新接口定义**: 新增了 `ILogSerializer` 和 `ILogConverter` 抽象接口。
* **验证步骤适配**: `StructureValidatorStep` 和 `LogicValidatorStep` 已更新，作为适配器调用新的 `TextValidator` 和 `LogicValidator` 组件，修复了变量命名冲突并对接了新的错误报告机制。
* **WorkflowHandler 重构**: 构造函数更新为接收接口指针（`std::shared_ptr<IInterface>`），移除了内部硬编码的 `new` 操作，解决了 vtable 链接错误。
* **PipelineFactory 更新**: 工厂方法现在通过参数接收外部注入的转换器和序列化器实例，不再自行实例化具体类。

#### 5. Importer Layer (导入层) [新增]

* **内部依赖解耦**: 所有的 `Importer` 内部组件不再负责创建其依赖对象，而是通过构造函数接收。
* `Writer`: 接收注入的 `ProjectResolver`，不再内部实例化，明确了职责边界。
* `Repository`: 接收注入的 `Connection`，将数据库连接管理与 CRUD 操作分离。
* `ImportService`: 接收注入的 `Repository` 和 `MemoryParser`，成为纯粹的业务编排层。


* **Facade 模式增强**: `data_importer.cpp` (Facade) 升级为子系统的 **Composition Root**。它负责实例化并组装上述所有组件，对外保持接口签名不变 (`handle_process_memory_data`)，从而确保了 Core 层无需修改即可享受重构成果。

#### 6. CLI Layer (命令行层)

* **AppContext 扩展**: 更新了上下文容器，新增了 `serializer` 和 `log_converter` 的组件槽位。
* **组装逻辑**: `CliApplication` 现在负责实例化具体的 `JsonSerializer` 和 `LogProcessor`，并将其注入到业务管线中。
