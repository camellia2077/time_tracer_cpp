...
* 重构 `GenericFormatterFactory`，移除了内部对具体 Formatter 类和 DLL 名称的硬编码映射。
* 引入**注册机制**，现在通过 `ReportService` 在启动时动态注册 DLL 映射关系。实现对修改关闭、对扩展开放 (OCP)，新增报表格式无需触碰工厂核心代码。


* **核心依赖解耦 (Dependency Decoupling)**:
* 消除了 `ProjectTreeBuilder`、`BatchDayDataFetcher` 等组件对 `ProjectNameCache` 单例的隐式内部调用。
* 新增 `IProjectInfoProvider` 接口，改用**依赖注入 (Dependency Injection)** 的方式传递项目信息上下文，明确了组件的输入边界，提升了模块的可测试性。



### 修复 (Fixes)

* 修正 `MonthTexUtils.hpp` 中因复制粘贴导致的错误命名空间及头文件定义。


## v0.4.0.8 (2026-01-14)

### reports和AppConfig解耦
- **Reports 模块与 IO 解耦**：Config 类不再直接读取磁盘文件，改为通过构造函数注入 JSON 数据。
- **接口标准化**：`create_formatter` 导出接口改为接收 JSON 字符串，不再依赖 `AppConfig`，实现了配置加载（IO/Core）与使用（Reports）的分离。
...
为了遵循单一职责原则并降低模块耦合度，对 `common/AppConfig.hpp` 进行了以下拆分和更新：

1. **文件结构拆分**：
* **配置定义** (`common/config/AppConfig.hpp`)：仅保留静态环境配置（如文件路径、`PipelineConfig`、`ReportConfig`），用于映射 `config.json`。
* **运行时选项** (`common/AppOptions.hpp`)：提取了 `AppOptions` 结构体，用于存储命令行参数（如 `run_all`, `date_check_mode` 等）。
* **结果模型** (`common/model/ProcessingResult.hpp`)：提取了 `ProcessingResult` 和 `ProcessingTimings`，用于传递处理结果数据。


2. **配置字段补全**：
* 在 `ReportConfig` 结构体中补全了缺失的 **Typst**、**LaTeX** 和 **Markdown** 的 `day` / `month` / `period` 配置路径字段，确保与 `config.json` 中的结构完全对应。


3. **模块适配与修复**：
* **ConfigLoader & StartupValidator**：更新了加载逻辑以支持新增的报告配置路径。
* **Include 路径修正**：全局更新了 `Core`、`CLI` 和 `Reports` 模块中对 `AppConfig` 的引用路径，并在需要的地方显式引入了新的 `AppOptions.hpp`。

## v0.4.0.10 (2026-01-14)

### 解耦config和bootstrap的文件读写

* **IO 解耦 (IO Decoupling)**:
* **核心配置加载**: 重构 `config/ConfigLoader`，移除了对 `<fstream>` 的直接依赖。现在配置文件的读取与存在性检查完全委托给 `io/core/FileReader` 和 `io/core/FileSystemHelper`，实现了配置模块与底层 IO 的解耦。
* **启动验证器**: 更新 `bootstrap/StartupValidator`，将内部辅助函数 `load_json_helper` 重写，使用 `FileReader::read_content` 替代 `std::ifstream` 进行 JSON 文件读取，统一了文件访问方式。
* **插件验证**: 修改 `config/validator/plugins/facade/PluginValidator`，将原有的 `std::filesystem` 直接调用替换为 `FileSystemHelper` 的封装方法，确保所有文件系统交互均受 `core/io` 模块控制。