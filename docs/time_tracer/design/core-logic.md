...
* **逻辑模型：为何选择截止时间 (End-Time Logic)**：
    * **原理**：系统将用户输入的每一行视为一个 **Checkpoint (检查点)**，而非 **Trigger (触发器)**。
    * **推导链**：
        * 用户输入：`1000 coding` (表示 10:00 结束了 coding)。
        * 系统推导：`Start_Time` = 上一条记录的 `End_Time`。
        * 系统推导：`Duration` = `Current_End` - `Last_End`。
    * **优势**：这种设计完美适配人类的短期记忆模式——我们通常在切换任务（如合上电脑、站起来去洗手间）的瞬间，才会有意识去记录刚才做完的事情。
