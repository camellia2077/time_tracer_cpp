...
void setup_console_for_utf8() {
    ...
    // This allows the program to use the terminal's configured encoding, which is usually UTF-8.
    try {
...
int main(int argc, char* argv[]) {
    // --- Setup ---
    setup_console_for_utf8(); // Set up console for UTF-8 output
    std::ios_base::sync_with_stdio(false);
    std::cin.tie(NULL);

    if (argc < 2) {
        std::cerr << RED_COLOR << "使用方法: " << argv[0] << " <输入文件.txt>" << RESET_COLOR << std::endl; // Usage message in Chinese
        return 1;
    }
    ...
    // Check if configuration files exist before proceeding
    if (!fs::exists(interval_config)) {
        std::cerr << RED_COLOR << "错误: 配置文件未找到: " << interval_config << "。程序即将退出。" << RESET_COLOR << std::endl;
        return 1;
    }
    if (!fs::exists(validator_config)) {
        std::cerr << RED_COLOR << "错误: 配置文件未找到: " << validator_config << "。程序即将退出。" << RESET_COLOR << std::endl;
        return 1;
    }
    ...
    bool process_ok = processor.processFile(input_file, processed_output_file);
    ...
    if (is_valid) {
        ...
        std::cerr << RED_COLOR << "检验失败。在生成的文件中发现错误: '" << processed_output_file << "'" << RESET_COLOR << std::endl;
