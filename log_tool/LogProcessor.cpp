...
bool LogProcessor::run(const AppOptions& options) {
    // 重置状态
    options_ = options;
    v_source_success_ = v_source_fail_ = 0;
    convert_success_ = convert_fail_ = 0;
    v_output_success_ = v_output_fail_ = 0;
...
bool LogProcessor::runAllInOneMode() {
    ...
    FormatValidator validator(config_.validator_config_path, config_.interval_config_path, options_.enable_day_count_check);
    IntervalProcessor processor(config_.interval_config_path);
